...
func main() {
	fmt.Println("📖 CometBFT 메시지 파일 파서")
	fmt.Println("============================")
...
func printUsage() {
	fmt.Println("사용법:")
	fmt.Println("  go run cmd/demo/message_file_parser.go <메시지파일경로>")
	fmt.Println("")
	fmt.Println("예제:")
	fmt.Println("  go run cmd/demo/message_file_parser.go examples/cometbft/samples.json")
	fmt.Println("  go run cmd/demo/message_file_parser.go examples/cometbft/all_messages.json")
	fmt.Println("  go run cmd/demo/message_file_parser.go examples/cometbft/Proposal.json")
	fmt.Println("")
	fmt.Println("사용 가능한 파일들:")
	printAvailableFiles()
}

func printAvailableFiles() {
	examplesDir := "examples/cometbft"
	if _, err := os.Stat(examplesDir); os.IsNotExist(err) {
		fmt.Println("  (examples 디렉토리가 없습니다. 먼저 예제를 생성해주세요)")
		return
	}
...
func parseMessageFile(messageFile string) {
	fmt.Println("\n📖 메시지 파일 파싱 중...")

	// 메시지 로드
	messages, err := loadMessagesFromJSON(messageFile)
	if err != nil {
	...
	for i, msg := range messages {
		...
		successCount++
		fmt.Printf("   ✅ 변환 성공!\n")
		printCanonicalDetails(canonical)
	}
	...
	// 변환 성공 메시지 출력
	if successCount > 0 {
		fmt.Printf("\n🎉 %d개 메시지가 성공적으로 Canonical 형식으로 변환되었습니다!\n", successCount)
	}
}
...
func printMessageDetails(msg abstraction.RawConsensusMessage) {
	fmt.Printf("   📋 원본 메시지 정보:\n")
...
func printCanonicalDetails(canonical *abstraction.CanonicalMessage) {
